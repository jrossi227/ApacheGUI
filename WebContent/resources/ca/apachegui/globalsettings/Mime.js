define([ "dojo/_base/declare",
	...
	declare("ca.apachegui.globalsettings.Mime", null, {
		...
		addListeners: function () {
			...
			on(registry.byId('searchServerMimeTypeButton'), "click", function() {
				...
				if(searchTypeTerm == '' ) {
					ca.apachegui.Util.alert("Error","You must enter a search term");
					return;
				}
			...
			on(registry.byId('addMimeSubmitButton'), "click", function() {
				...
				if(type == '') {
					ca.apachegui.Util.alert('Error','You must specify a type');
					return;
				}
				
				if(extensions == '') {
					ca.apachegui.Util.alert('Error','You must specify an extension');
					return;
				}
			...
			on(registry.byId('removeMimeMenuItem'), "click", function() {
				...
				ca.apachegui.Util.confirmDialog("Please Confirm", "Are you sure you want to delete the follwing MIME?<br/>" +
	...
	ca.apachegui.globalsettings.Mime.currentMimes=null;
	//used globally to grab instance
	ca.apachegui.globalsettings.Mime.getInstance = function() {
		if(!ca.apachegui.globalsettings.Mime.currentMimes) {
			ca.apachegui.globalsettings.Mime.currentMimes=new ca.apachegui.globalsettings.Mime();
		}
		
		return ca.apachegui.globalsettings.Mime.currentMimes;
	};
