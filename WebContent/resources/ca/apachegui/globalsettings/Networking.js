define([ "dojo/_base/declare",
	...
	declare("ca.apachegui.globalsettings.Networking", null, {
		...
		addListenSubmit: function() {
			...
			if(allIp == false &&  ip == "") {
				ca.apachegui.Util.alert('Error','You must specify either All IP Addresses or a specific IP Address.');
				return;
			}
			
			if(port == "") {
				ca.apachegui.Util.alert('Error','You must specify a Port');
				return;
			}
			
			if(isNaN(port)) {
				ca.apachegui.Util.alert('Error','The Port must be numeric');
				return;
			}
			...
			if(allIp == false &&  !isValidIp) {
				ca.apachegui.Util.alert('Error','The IP Address you entered is not valid.');
				return;
			}
		...
		addNameVirtualHostSubmit: function() {
			...
			if(address == "") {
				ca.apachegui.Util.alert('Error','You must specify an address');
				return;
			}
		...
		showSuccessfullSave: function() {
			ca.apachegui.Util.alert('Success','The changes were successfully saved');
		},
		...
		addListeners: function() {
			...
			on(registry.byId('removeListenMenuItem'), "click", function() {
				...
				ca.apachegui.Util.confirmDialog("Please Confirm", "Are you sure you want to delete the Listener with the following properties?<br/>" +
			...
			on(registry.byId('removeNameVirtualHostMenuItem'), "click", function() {
				...
				ca.apachegui.Util.confirmDialog("Please Confirm", "Are you sure you want to delete the Name Based Virtual Host with the following properties?<br/>" +
			...
			on(registry.byId('saveKeepAliveTimeoutButton'), "click", function() {
				...
				if(seconds == '' ) {
					ca.apachegui.Util.alert('Error','You must specify a value for seconds');
					return;
				}
					
				if(isNaN(seconds)) {
					ca.apachegui.Util.alert('Error','The specified seconds must be numeric');
					return;
				}
			...
			on(registry.byId('requestsPerConnectionButton'), "click", function() {
				...
				if(numberOfRequests == '' ) {
					ca.apachegui.Util.alert('Error','You must specify a value for Requests per Connection');
					return;
				}
					
				if(isNaN(numberOfRequests)) {
					ca.apachegui.Util.alert('Error','The specified Requests per Connection must be numeric');
					return;
				}
			...
			on(registry.byId('listenBackLogButton'), "click", function() {
				...
				if(backLogLength == '' ) {
					ca.apachegui.Util.alert('Error','You must specify a value for the Queue Length');
					return;
				}
					
				if(isNaN(backLogLength)) {
					ca.apachegui.Util.alert('Error','The specified Queue Length must be numeric');
					return;
				}
			...
			if(!ca.apachegui.Main.getInstance().isWindows()) {
				...
				on(registry.byId('apacheUserButton'), "click", function() {
					...
					if(user == '' ) {
						ca.apachegui.Util.alert('Error','You must specify a value for user');
						return;
					}
				...
				on(registry.byId('apacheGroupButton'), "click", function() {
					...
					if(group == '' ) {
						ca.apachegui.Util.alert('Error','You must specify a value for group');
						return;
					}
	...
	ca.apachegui.globalsettings.Networking.currentNetworking=null;
	//used globally to grab instance
	ca.apachegui.globalsettings.Networking.getInstance = function() {
		if(!ca.apachegui.globalsettings.Networking.currentNetworking) {
			ca.apachegui.globalsettings.Networking.currentNetworking=new ca.apachegui.globalsettings.Networking();
		}
		
		return ca.apachegui.globalsettings.Networking.currentNetworking;
	};
