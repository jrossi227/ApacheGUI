define([ "dojo/_base/declare",
	...
	declare("ca.apachegui.Editor", null, {	
		...
		init: function() {
			...
			var storedTheme=ca.apachegui.Settings.getInstance().getSetting(ca.apachegui.Settings.getInstance().settingsMap.editorTheme);
			if(storedTheme=='') {
			...
			var storedShowTabs=ca.apachegui.Settings.getInstance().getSetting(ca.apachegui.Settings.getInstance().settingsMap.showTabs);
		...
		startUpdateTimer: function() {
			...
			this.refreshUpdateTimerHandler = ca.apachegui.Interval.setInterval(this.checkLastModifiedTime.bind(this), 5000);	
		},
		
		stopUpdateTimer: function() {
			ca.apachegui.Interval.clearInterval( this.refreshUpdateTimerHandler );
		},
		...
		setTheme: function(theme) {
			...
			if(!!this.currTheme) {
				...
				ca.apachegui.Settings.getInstance().setSetting(ca.apachegui.Settings.getInstance().settingsMap.editorTheme, theme);
			}
		...
		showTabs: function() {
			...
			if(this.tabsShown==false) {
				ca.apachegui.Settings.getInstance().setSetting(ca.apachegui.Settings.getInstance().settingsMap.showTabs, 'true');
			}
		...
		hideTabs: function() {
			...
			if(this.tabsShown==true) {
				ca.apachegui.Settings.getInstance().setSetting(ca.apachegui.Settings.getInstance().settingsMap.showTabs, 'false');
			}
		...
		newFileSubmit: function () {
			...
			request.post("../Menu", {
				...
				else
				{
					...
					ca.apachegui.Menu.getInstance().refresh();
		...
		setEditorFromFile: function (url, file, callback) {
			...
			var thisdialog = ca.apachegui.Util.noCloseDialog('Loading', 'Loading File Please Wait...');
			thisdialog.show();
			...
			request.get(url, {
				...
				function(error) {
					thisdialog.remove();
					ca.apachegui.Util.alert('Error',error.response.data);
				}
			);
		},
		...
		addListeners: function() {
			...
			on(window, "beforeunload", function(e) {
				if(that.saveState==true && ca.apachegui.Main.getInstance().isSessionActive()) {
			...
			CodeMirror.commands.autoFormat= function(){
		    	if(that.isFormattable()) {
					...
					ca.apachegui.Util.alert('Unsupported',unsupportedMessage);
				}
		    };
