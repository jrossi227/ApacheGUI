define([ "dojo/_base/declare",
	...
	declare("net.apachegui.GUISettings", null, {
		...
		isUpdateAvailable: function(newVersion) {
			var currentVersionNum=parseInt(net.apachegui.Main.getInstance().getApacheGuiVersion().split('.').join(''));
				...
				request.post("/ApacheGUIUpdater/Update", {
					...
					function(response) {
						...
						if(!!data.version && that.isUpdateAvailable(data.version)) {
							...
							if(net.apachegui.Main.getInstance().getApacheGuiVersion().match(new RegExp(data.compatibilitys,"g")) == null){
		...
		updateGUI: function() {
			...
			request.post("/ApacheGUIUpdater/Update", {
				...
				function(response) {
					...
					that.updateTimer = net.apachegui.Interval.setInterval(that.getUpdateStatus.bind(that), 3000);	
						
				},
				function(error) {
					net.apachegui.Util.alert('Error',"There was an error starting the update");
				}
			);
		},
		...
		getUpdateStatus: function() {
			...
			request.post("/ApacheGUIUpdater/Update", {
				...
				function(error) {
					net.apachegui.Util.alert('Error',"There was an error with the update");
				}
			);
		},
		
		clearUpdateTimer: function() {
			net.apachegui.Interval.clearInterval ( this.updateTimer );
		},
