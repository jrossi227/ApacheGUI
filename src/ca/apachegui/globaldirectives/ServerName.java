...
public class ServerName extends SingletonDirective {
	...
	/**
	 * Creates a ServerName object from the directive. 
	...
	/**
	 * Static function to get the current configured ServerName in apache.
	 * If there is no ServerName found then a ServerName is returned with an empty domain.
	 * 
	...
	public ServerName getConfigured() throws Exception {
		String serverName[]=new DirectiveParser(SettingsDao.getInstance().getSetting(Constants.confFile), SettingsDao.getInstance().getSetting(Constants.serverRoot), StaticModuleHandler.getStaticModules(), SharedModuleHandler.getSharedModules()).getDirectiveValue(directiveName, false);
