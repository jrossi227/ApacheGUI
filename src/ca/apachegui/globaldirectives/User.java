...
public class User extends SingletonDirective{
	...
	/**
	 * Creates a User object from the directive. 
	...
	/**
	 * Static function to get the current configured User in apache.
	 * If there is no User found then a User is returned with an empty value.
	 * 
	...
	User getConfigured() throws Exception {
		String user[]=new DirectiveParser(SettingsDao.getInstance().getSetting(Constants.confFile), SettingsDao.getInstance().getSetting(Constants.serverRoot), StaticModuleHandler.getStaticModules(), SharedModuleHandler.getSharedModules()).getDirectiveValue(directiveName, false);
