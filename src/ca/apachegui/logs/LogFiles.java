...
public class LogFiles 
{
	 ...
	 * 
	 * @param path - the path of the target log files. The path is relative to the root log file directory.
	...
	public static String getNodeJson(String path) 
	{
		...
		File child;
		   for(int i=0; i<children.length; i++)
		   {
			  ...
			  {
				  result.append("{ $ref: '" + Constants.LogsRoot + child.getAbsolutePath() + "', id:'" + Constants.LogsRoot +  child.getAbsolutePath() + "', type:'Logs', name: '" + child.getName() + "', children: true},");
			  }
		   }
		   for(int i=0; i<children.length; i++)
		   {
