...
public class ConfFiles
{
	 ...
	 * 
	 * @param message - the meassage to write to the file
	 * @throws IOException if root ConfigFile can't be found
	 */
	public static void appendToRootConfigFile(String message) throws IOException
	{
		...
		String confFile=Settings.getSetting(Constants.confFile);
	...
	/**
	 * Writes a newline then message to the end of the gui specific config file. 
	 * An include is written in the main config file if the gui Config file has not yet been included.
	 * 
	...
	/**
	 * Searches for lines that match a specific Regex in the gui config file. 
	 * If the Regex is found then the lines are removed from the gui file.
	 * 
	 * @param regex - lines matching the regex will be removed from the file
	...
	public static void removeFromGUIConfigFile(String regex) throws Exception
	{
		...
		File guiFile=new File(Settings.getSetting(Constants.confDirectory),Constants.guiConfFile);
	...
	/**
	 * Deletes all lines matching a regex only from active config files.
	 * Will not delete commented results.
	 * 
	 * @param regex - lines matching this regex will be removed from active config files
	 * @throws Exception if a file error occurs
	 */
	...
	/**
	 * Searches active config files for the existence of a regex. 
	 * This method will not return commented results.
	 * 
	...
	/**
	 * Gets all Conf Files in the Configuration Directory. 
	 * Some apache configurations use the server root as the root configuration directory. 
	 * As a result the root configuration directory can include the folder for modules logs and run. 
	 * Any files contained in a folder logs modules and run will not be included in the results. 
	 * 
	 ...
	 * 
	 * @param path - the path of the target config files.
	...
	public static String getNodeJson(String path) 
	{
		...
		File confDirectory=new File(Settings.getSetting(Constants.confDirectory));
		
		StringBuffer result= new StringBuffer();
		result.append("{ id: '" + Constants.ConfigurationRoot + targetDirectory.getAbsolutePath() + "', name:'" + (targetDirectory.equals(confDirectory) ? "Configuration" : targetDirectory.getName()) + "', type:'Configuration', children:[");
	    ...
	    for(int i=0; i<children.length; i++)
	    {
		   ...
		   {
			   if(!child.getAbsolutePath().matches(Settings.getSetting(Constants.confDirectory) + Constants.sanitizedConfigFiles)) {
				   result.append("{ $ref: '" + Constants.ConfigurationRoot + child.getAbsolutePath() + "', id:'" + Constants.ConfigurationRoot +  child.getAbsolutePath() + "', type:'Configuration', name: '" + child.getName() + "', children: true},");
			   }
		   }
	    }
	    for(int i=0; i<children.length; i++)
	    {
		   ...
		   {
			   if(!child.getAbsolutePath().matches(Settings.getSetting(Constants.confDirectory) + Constants.sanitizedConfigFiles)) {
				   result.append("{ $ref: '" + Constants.ConfigurationRoot +  child.getAbsolutePath() + "', id:'" + Constants.ConfigurationRoot +  child.getAbsolutePath() + "', type:'Configuration', name: '" + child.getName() + "'},");
			   }
		   }
	    }
	 ...
	 * Some apache configurations use the server root as the root configuration directory. 
	 * As a result the root configuration directory can include the folder for modules logs and run. 
	 * Any files contained in a folder logs modules and run will be removed from confFiles. 
	 * 
	 * @param confFiles - the list of confFiles to sanitize
	 */
